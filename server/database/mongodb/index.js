...
mongoose.connect('mongodb://127.0.0.1/junglegym');
...
const updateUser = async (_id , body) => {
  // console.log(_id, body,'eeeeeeeeeeeeeeeeeeeeeeeeeeeeee');
  try {
    const updatedUser = await UserModel.findByIdAndUpdate({_id : _id},body);
    return updatedUser;
  } catch (error) {
    console.error(error);
  }
...
const createexercices = async (exeData) => {
  try {
    const exercices = new exe (exeData)
    const result = await exercices.save();
    return result;
  }
  catch (error) {
    console.error(error);
...
const getAllexercices = async () => {
  try{
    const exercices = await exe.find();
    return exercices;
  }catch(error){
console.error(error)
  }
  }
  const updateexercices = async (_id, body) => {
    // console.log(_id, body,'eeeeeeeeeeeeeeeeeeeeeeeeeeeeee');
    try {
      const updatedUser = await exe.findByIdAndUpdate({_id : _id},body);
      return updatedUser;
    } catch (error) {
      console.error(error);
    }
  }
  const deleteexercices = async (id) => {
    try {
      const deletedUser = await exe.findByIdAndDelete(id);
      return deletedUser;
    } catch (error) {
      console.error(error);
    }
